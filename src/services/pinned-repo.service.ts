...
export const pinnedRepoService = {
  async getPinnedRepos(): Promise<DataResponse<ListData<PinnedRepo>>> {
    const { data } = await apiClient.get<DataResponse<ListData<PinnedRepo>>>('/v1/pinned-repos');
    return data;
  },

  async pinRepo(request: PinRepoRequest): Promise<DataResponse<PinnedRepo>> {
    const { data } = await apiClient.post<DataResponse<PinnedRepo>>('/v1/pinned-repos', request);
    return data;
  },
