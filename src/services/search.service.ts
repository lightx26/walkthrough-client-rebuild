...
export const searchService = {
  async searchWalkthroughs(
    ...
    const { data } = await apiClient.post<DataResponse<SearchWalkthroughResponse>>(
      '/v1/search/walkthroughs',
      request
    );
    return data;
  },
  ...
  async getHistory(): Promise<DataResponse<ListData<SearchHistoryItem>>> {
    const { data } =
      await apiClient.get<DataResponse<ListData<SearchHistoryItem>>>('/v1/search/history');
    return data;
  },
