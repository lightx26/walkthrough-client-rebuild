...
export const analyticsService = {
  async authorSummary(params?: {
    ...
    const { data } = await apiClient.get<DataResponse<ListData<AuthorWalkthroughSummary>>>(
  ...
  async reviewProgress(walkthroughId: string): Promise<DataResponse<ReviewProgress>> {
  ...
  async chapterAttention(walkthroughId: string): Promise<DataResponse<ChapterAttentionResponse>> {
    const { data } = await apiClient.get<DataResponse<ChapterAttentionResponse>>(
      `/v1/analytics/walkthroughs/${walkthroughId}/chapter-attention`
    );
    return data;
  },

  async unreadSummary(walkthroughId: string): Promise<DataResponse<UnreadSummary>> {
