    ...
    <div className="rounded-lg border border-gray-200 bg-white">
      <div className="flex items-center justify-between px-5 pt-5 pb-3">
        <h2 className="text-sm font-semibold text-gray-900">
          ...
          {total != null && <span className="font-normal text-gray-400">{total}</span>}
        </h2>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="px-5 pb-5 text-sm text-gray-400">No pull requests found.</p>
      ) : (
        <div className="px-5 pb-2">
          {items.map((pr) => (
            <PrRow key={pr.id} pr={toPrRowItem(pr)} owner={pr.owner} repo={pr.repo} showRepoInfo />
          ))}
        </div>
      )}
    </div>
